...
msg() {
    if [ "${USE_CHINESE}" = true ]; then
        ...
        case "$1" in
            ...
            "fix_permentsions") echo "Fixing WSLg permissions..." ;;
...
# 应用自定义分辨率
xrandr --output XWAYLAND0 --mode ${RES_WIDTH}x${RES_HEIGHT} >/dev/null 2>&1 || true
EOF
    
    # 替换分辨率占位符
    sed -i "s/\${RES_WIDTH}/$RES_WIDTH/g" ~/.config/scale
    sed -i "s/\${RES_HEIGHT}/$RES_HEIGHT/g" ~/.config/scale
    ...
    # 在Xwayland脚本中硬编码分辨率
    if [ -f "/usr/bin/Xorg.Xwayland" ]; then
...
if [ -f ~/.config/scale ]; then
    echo "Applying display scaling settings..."
    ~/.config/scale
fi
